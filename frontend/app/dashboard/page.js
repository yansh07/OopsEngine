...
export default function Dashboard() {
  ...
  async function fetchHistory(userId) {
    try {
      const response = await fetch(`https://${process.env.NEXT_PUBLIC_API_URL}/api/execute/history/${userId}`);
    ...
    if (isLoaded && isAuthLoaded && user) {
      const syncUser = async () => {
        try {
          ...
          const response = await fetch(`https://${process.env.NEXT_PUBLIC_API_URL}/api/auth/sync`, {
            method: "POST",
            headers: {
  ...
  const runCode = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://${process.env.NEXT_PUBLIC_API_URL}/api/execute/`, {
        method: "POST",
      ...
      while (result.status === "QUEUED" || result.status === "RUNNING") {
        ...
        const pollResponse = await fetch(`https://${process.env.NEXT_PUBLIC_API_URL}/api/execute/${result.id}`, {
            headers: {
                "Authorization": `Bearer ${token}`
            }
        });
